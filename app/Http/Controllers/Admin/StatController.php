...
//    public function getOrder(Request $request)
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.discharge.index',compact('user_id'));
    }
...
//        $builder = StatUser::orderBy('record_at', 'DESC')->where('user_id', $request->input('user_id'));
        ...
        $stat_user=$builder->paginate($request->get('limit',30))->toArray();


        foreach($stat_user['data'] as &$val){
            $val['record_at']=date('Y-m-d',$val['record_at']);
            $val['server_rate']=sprintf("%.2f", $val['server_rate']);
            $val['u']=$this->getFlow($val['u']);
            $val['d']=$this->getFlow($val['d']);
        }

        $data = [
            'code' => 0,
            'msg'   => '正在请求中...',
            'count' => $stat_user['total'],
            'data'  => $stat_user['data']
        ];
        return response()->json($data);
    }

    protected function getFlow($num){

        if($num<=1024){
            $res=sprintf("%.2f", $num).' B';
        }elseif ($num>1024 && $num<=1048576){
            $res=sprintf("%.2f", $num/1024).' K';
        }elseif ($num>1048576 && $num<=1073741824){
            $res=sprintf("%.2f", $num/(1024*1024)).' MB';
        }else{
            $res=sprintf("%.2f", $num/(1024*1024*1024)).' GB';
        }
