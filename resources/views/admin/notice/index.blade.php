    ...
    <div class="layui-card">
        <div class="layui-card-header layuiadmin-card-header-auto">
            <div class="layui-btn-group ">
                @can('v2board.notice.create')
                    <a class="layui-btn layui-btn-sm" href="{{ route('admin.notice.create') }}">添加公告</a>
                @endcan
            </div>
        ...
        <div class="layui-card-body">
            ...
            <script type="text/html" id="options">
                <div class="layui-btn-group">
                    @can('v2board.notice.edit')
                        <a class="layui-btn layui-btn-sm" lay-event="edit">编辑</a>
                    @endcan
                    @can('v2board.notice.destroy')
                ...
                table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'del'){
                        layer.confirm('确认删除吗？', function(index){
                            $.post("{{ route('admin.notice.destroy') }}",{_method:'delete',ids:[data.id]},function (result) {
                                if (result.code==0){
