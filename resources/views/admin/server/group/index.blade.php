    ...
    <div class="layui-card">
        <div class="layui-card-header layuiadmin-card-header-auto">
            <div class="layui-btn-group ">
                @can('server.group.destroy')
                    {{--<button class="layui-btn layui-btn-sm layui-btn-danger" id="listDelete">删 除</button>--}}
                @endcan
                @can('server.group.create')
                    <a class="layui-btn layui-btn-sm" href="{{ route('admin.group.create') }}">添 加</a>
                @endcan
        ...
        <script>
            layui.use(['layer','table','form'],function () {
                ...
                var dataTable = table.render({
                    elem: '#dataTable'
                    ,height: 500
                    ,url: "{{ route('admin.group.data') }}" //数据接口
                    ,page: true //开启分页
                    ,cols: [[ //表头
                        // {checkbox: true,fixed: true}
                        {field: 'id', title: '组ID', sort: true,width:80}
                        ,{field: 'name', title: '标题'}
                        ,{field: 'user_count', title: '用户数量'}
                        ,{field: 'server_count', title: '节点数量'}
                ...
                table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'del'){
                        layer.confirm('确认删除吗？', function(index){
                            $.post("{{ route('admin.group.destroy') }}",{_method:'delete',ids:[data.id]},function (result) {
                                if (result.code==0){
                        ...
                        location.href = '/admin/group/'+data.id+'/edit';
                    }
                });
                ...
                $("#listDelete").click(function () {
                    ...
                    if (ids.length>0){
                        layer.confirm('确认删除吗？', function(index){
                            $.post("{{ route('admin.group.destroy') }}",{_method:'delete',ids:ids},function (result) {
                                if (result.code==0){
                                    dataTable.reload()
                                }
                                layer.close(index);
